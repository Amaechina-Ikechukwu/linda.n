...
export default function PropertyInfo({ property }: { property: OfferData }) {
    ...
    return (
        <div className='sm:flex  sm:justify-between space-y-[20px] sm:space-x-[100px] mt-[80px]'>
            <img src={property.image_url} className='w-full sm:w-2/5 h-fit ' />
            <div className='max-w-2xl space-y-[20px] sm:space-y-[40px]'>
                ...
                <div className='p-4 rounded-md bg-slate-100 dark:bg-neutral-950 space-y-[10px]'>
                    ...
                    {property.terms.split(".").map((term: string, index: number) => (
                        <div className='flex space-x-4'><h5 className='text-xs text-gray-700 dark:text-slate-100'>{index + 1 + "."}</h5><h5 className='text-xs text-gray-700 dark:text-slate-100'>{term}</h5></div>

                    ))}
                </div>
                <div className='space-y-[20px] sm:space-y-[20px]'>
                    <h5 className='text-xs text-gray-700 dark:text-slate-100 text-[30px] font-bold'>{property.currency + property.amount}</h5><h5 className='text-xs text-gray-700 dark:text-slate-100'>{`(Instead of ${property.currency + property.amount}, offer ends soon)`}</h5>
                    <div className='space-y-[10px] sm:space-y-[10px] mt-[20px] items-center'><h5 className='text-xs text-gray-700 dark:text-slate-100'>Interested in this property?</h5>
